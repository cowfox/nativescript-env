...
help: ## Show this help message
	...
	@echo ""
	@echo "发布流程 (publish to npmjs.org, tag 触发 CI 发布):"
	@echo "  1. develop:   make cut [minor|major]      → 创建 release/<ver> 分支"
	@echo "  2. release/*: make pre beta (或 alpha/rc)  → 进入预发布模式"
	@echo "  3.            make changeset auto && git commit"
	@echo "  4.            make release push           → bump 版本 + commit"
	@echo "  5.            make tag push               → 创建并推送 v<ver> tag → CI 自动发布"
	@echo "  6. 最终版:    make pre exit && make release push && make tag push"
	@echo ""
