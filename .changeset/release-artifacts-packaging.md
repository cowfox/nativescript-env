...
- **iOS**: Archives `.xcarchive` directly into structured `dist/v{version}_b{buildNumber}/` directories, collects `.ipa` packages, packages `.dSYM` debug symbols to `.zip`, and provides optional automatic upload to Firebase Crashlytics via `upload-symbols`.
